...
export async function requestPublicResourcePersistently(input: {
  ...
  try {
    ...
    const response = await pRetry(() => requestOneAttempt(input), {
      retries: 1,
      minTimeout: Math.max(10_000, source.accessPolicy.minimumIntervalMs),
      maxTimeout: Math.max(10_000, source.accessPolicy.minimumIntervalMs),
      factor: 1,
      randomize: false,
      ...(input.signal ? { signal: input.signal } : {}),
      shouldRetry: ({ error }) => isTransientTransportFailure(error),
    });
...
async function requestOneAttempt(input: Parameters<typeof requestPublicResourcePersistently>[0]) {
  ...
  let attempt: Awaited<ReturnType<typeof reserveWhenEligible>> | undefined = await reserveWhenEligible(
    source, runId, admission, targetKey, workKey, currentUrl, input.signal);
  ...
  try {
    const response = await input.request(url, input.maximumBytes, input.signal, async (event) => {
      ...
      attempt = await reserveWhenEligible(source, runId, admission, targetKey, workKey,
        currentUrl, input.signal, redirectParentAttemptId);
    });
...
async function reserveWhenEligible(source: CrawlPlanSource, runId: string,
  ...
  signal?: AbortSignal, redirectParentAttemptId?: string) {
  while (true) {
    const result = await admission.reserveRequest({ runId, targetKey, workKey,
      ...
      gateKey: `${source.provider.key}@${source.provider.version}:${url.origin}`,
...
function isTransientTransportFailure(error: unknown) {
  ...
  return /socket disconnected before secure TLS connection|HTTP\/2.*internal error|公共资源请求超时/i
    .test(error.message);
}
