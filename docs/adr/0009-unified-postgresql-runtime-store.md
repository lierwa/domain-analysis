...
> 2026-08-26：上述退出仍适用于旧知识流水线和 `domain_analysis_pipeline` schema；ADR 0020 另行接受 Crawl Planning 专用 DBOS adapter，并使用独立 `domain_analysis_crawl_planning` schema，不恢复旧业务模块。
