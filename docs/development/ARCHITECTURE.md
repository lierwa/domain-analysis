...
状态：2026-08-18 已收口到两阶段；当前只实施阶段 1 数据抓取
...
| 用户确认的取舍 | Interview Decision | Skill 提建议；UI 发起确认 |
...
- 接受用户首句，不重复确认已经明确的门类；
- 保存消息、2–3 个推荐选项的负责人问题、决定和未决项；
- 将结构化问题统一保存为 proposed Decision；用户点击某个选项就是对该项的显式确认，不再增加第二个“确认”动作；
...
5. 运行中逐 token 展示 Agent 的真实中文 commentary，并逐条展示经过脱敏的真实搜索/工具活动；不能用固定状态文案或打字机效果冒充流式，也不能从搜索状态倒退回泛化“分析中”；
6. 点击结构化选项即确认实际点击项并继续，不出现第二个“显式确认”；
7. 用户确认后生成正式 Capture Task，但不自动抓取；已确认任务可以继续原对话并生成同一任务的新版本。
