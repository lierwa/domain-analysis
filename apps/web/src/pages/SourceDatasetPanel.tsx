  ...
  return (
    <section className="grid gap-5 lg:grid-cols-[260px_minmax(0,1fr)]">
      ...
      <article className="min-w-0 rounded-xl border border-line bg-surface p-5">
        ...
        {detail.data && <>
          <div className="mb-5 flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-semibold">原始来源数据</h3><p className="mt-1 text-xs text-muted">{detail.data.records.length} 个不可变快照 · {renderRunStatus(detail.data.run)}</p></div><div className="flex gap-2"><a className="button-secondary" href={sourceRunExportUrl(taskId, detail.data.run.id, "jsonl")}><Download className="h-4 w-4" />JSONL</a><a className="button-secondary" href={sourceRunExportUrl(taskId, detail.data.run.id, "csv")}><Download className="h-4 w-4" />CSV</a></div></div>
