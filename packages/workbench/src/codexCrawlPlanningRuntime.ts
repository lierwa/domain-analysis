...
function planningPrompt(input: Parameters<CrawlPlanningRuntime["run"]>[0]) {
  ...
  const candidateChecklist = input.task.content.sourceCandidates.map((candidate) => ({
    ...
    requiredProvider: candidate.sourceKind === "retailer" && new URL(candidate.entryUrl).hostname === "www.jd.com"
      ? "jd.catalog-product@2.0.0; keep this candidate in its own source with five all_available targets: catalog_pages, store_catalogs, product_details, review_summaries, review_samples"
  ...
  return [
    ...
    "If Capture Task jd.applicable=true and jd.disposition=included, the plan MUST contain an additional jd.catalog-product@2.0.0 source even when the interviewed JD candidate is search.jd.com and must remain public.web-resource. A generic JD search/root snapshot does not satisfy JD product coverage. Use web search to find a precise anonymous www.jd.com catalog entry; if none can be verified, do not return a falsely complete plan.",
    "Provider binding checklist: follow Candidate execution checklist.requiredProvider exactly and leave every executionBlockers array empty. In particular, search.jd.com is public.web-resource and MUST NOT use jd.catalog-product. jd.catalog-product@2.0.0 only accepts www.jd.com HTTPS catalog entries and source configuration keys mode=explicit_http/include_text/exclude_text. Each JD source has exactly five all_available targets, one for each operation=catalog_pages/store_catalogs/product_details/review_summaries/review_samples; review_samples additionally freezes samples_per_product=100. JD accessPolicy is version=jd-explicit-http-v2, maxRequestsPerMinute=1, minimumIntervalMs=60000; raw formats are html+source_json, retainAssets=false, and requestBudget must be an explicit bounded hop limit of at least entryUrls count plus four. mall.jd.com and every other non-www.jd.com exact HTTPS entry use public.web-resource@1.0.0. A public source configuration is exactly mode=exact_https plus integer maximum_bytes (no url key there). Every exact public target configuration url MUST appear in that same source.entryUrls, and every source.entryUrls value has exactly one exact target; never borrow or duplicate another candidate's URL as a target in this source. When an expected manual/table attachment URL is only present in the entry HTML, add a later linked target whose configuration is exactly from_target=<earlier target key> plus link_text=<complete unique anchor text>; it may follow only once and same-origin. Every public target is target_count=1, and requestBudget is at least total target count plus unique exact-entry origin count for robots.txt. Never emit provider_missing.",
