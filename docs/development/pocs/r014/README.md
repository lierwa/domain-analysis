...
状态：首轮确定性抽取与 Codex 候选已通过，身份隔离和冲突门仍在验证
目标阶段：1B
调查日期：2026-08-14
...
1. 接入 I05/I06 已脱敏京东投影，验证不同品牌/型号不会被混入当前主体，并保留下架状态和缺失资料。
2. 用受控、明确标注为测试数据的同身份同属性双值 fixture 验证冲突进入人工例外；真实样本仍保持 0 冲突。
3. 增加审核接受/拒绝记录和“模型结果不能直接发布”的可执行门，再决定 1B 是否通过。
