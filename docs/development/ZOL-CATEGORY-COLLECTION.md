...
ZOL HTML 页面另允许一次来源特有的 404 复核：真实 Windows 抓取已经证明同一参数页或图集页可先返回 404、随后在同一正式代理路径返回 200。第一次 404 结束并记录当前 Request Attempt，等待现有重试间隔后重新进入 Source Access Gate；第二次仍为 404 时不再重试，由 Provider 保存最终 `not_found` 原始响应并隔离当前品牌或型号。图片 Asset、其他 Provider、401/403/429、robots、预算和取消不采用该策略。
