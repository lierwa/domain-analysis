...
import { closeDb, createDb, initializeDatabase } from "./client";
import {
...
describe("topic repository", () => {
  ...
  it("creates, lists, and updates topics", async () => {
    ...
    try {
      ...
      expect(await topics.getById(created.id)).toMatchObject({ id: created.id, name: "AI Search Trends" });
      expect(await topics.getById("topic_nonexistent")).toBeNull();
    } finally {
      closeDb(db);
    }
  });
...
describe("source repository", () => {
  it("seeds default sources and toggles enabled state", async () => {
    ...
    try {
      ...
      const reddit = await sources.updateByPlatform("reddit", { enabled: false });
