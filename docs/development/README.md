...
- Profile、Cookie、认证 Header、Codex 登录材料和受限证据不进入 Git；跨电脑同步只包含可公开的代码、配置、文档、测试和 contract。
- 两台开发机各自保留 PostgreSQL、Evidence CAS 和知识包运行数据，不通过 Git 搬运或合并；新电脑允许从空库继续开发。
...
当前阶段和下一步以 `PROGRESS.md` 为准。阶段 0I 的系统内采访 Agent、ephemeral Session 隔离与 PC 主流程已经通过；6.2 Market Universe 公共 contract、确认事务和 PC 投影已经通过，当前只推进 6.3 品牌官网与监管同窗分母。能效备案按已知官网型号交叉，不按不可信的监管 `total` 反向枚举市场总体；批量执行必须接入既有 durable pipeline，不能塞进一次同步 HTTP 刷新。
