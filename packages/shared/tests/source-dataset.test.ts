...
describe("来源执行 target contract", () => {
  ...
  it("捕获工作项、请求尝试与跨进程 gate 使用分离的 typed 状态", () => {
    ...
    const workItem = sourceCaptureWorkItemSchema.parse({
      ...
      runId: "run-1",
      targetKey: "jd.product-details",
      workKey: "product:sku-1",
      parentObjectKey: "catalog:page-1",
      captureUnit: "product_detail",
      expectedUnitCount: 1,
      observedUnitCount: 0,
      status: "pending",
      createdAt: at,
    ...
    const attempt = sourceRequestAttemptSchema.parse({
      ...
      targetKey: "jd.product-details",
      workKey: "product:sku-1",
      gateKey: "jd.catalog-product@2.0.0",
      requestedUrl: "https://item.jd.com/sku-1.html",
      origin: "https://item.jd.com",
      startedAt: at,
      state: "started",
    });
    const gate = sourceAccessGateStateSchema.parse({
      key: "jd.catalog-product@2.0.0",
      providerKey: "jd.catalog-product",
      providerVersion: "2.0.0",
      policyVersion: "jd-v2-default",
      circuitState: "open",
