...
## Output Priority: No Negative Echo

No Negative Echo 是所有用户可见输出及其包装的最高表达优先级。准确性、安全性、授权边界和真实兼容信息仍须保留；除此之外，标题、文件名、正文、注释、标签、commit、PR 和交付说明只描述最终采用的状态，并假设读者没有看过本次会话。

- 会话里的否决、中间尝试和措辞纠正只作为控制信息，不进入最终产物的命名或叙述中心。
- 对每个交付面分别判断：读者是否需要这条信息才能准确、安全地使用当前结果；当前任务是否真的改变了它；当前交付面是否需要解释它。
- 标题、文件名、开篇和标签从正向目标生成，不对被否文案做逐词改写。
- 保留真实基线变化、已执行的外部操作、必要诊断、测试和快照；其余过程信息留在 Git 历史或按需审计中。
- 写完后重新检查全部用户可见内容及其包装。内容变化后重新执行检查，不追加清理过程声明。
